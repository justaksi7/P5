...
void DepthFirstSearch(Digraph<TV, maxVertices> &g) { // Tiefensuche
    int time = 1;
...
// Breitensuche
template <class TV, int maxVertices>
void Bfs(Digraph<TV, maxVertices> &g, int v) { // Breitensuche ab Knoten v
...
template <class TV, int maxVertices>
void BreadthFirstSearch(Digraph<TV, maxVertices> &g) { // Breitensuche
    setAllColor(g, WHITE); // Alle Knoten als unbesucht markieren
    int v = g.firstVertex();
    while (v != -1) {
        if (!g.isVisited(v)) // v noch nicht besucht
